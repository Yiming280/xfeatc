...
{
	int ret = OPT_OK;
	OPT_HANDLE devHandle = (OPT_HANDLE)pUserData;
	OPT_Frame frame;
	...
	while (!g_isExitThread)
	{
		// 获取一帧图像
		// Get a frame image
		ret = OPT_GetFrame(devHandle, &frame, 500);
		if (OPT_OK != ret)
		{
...
static void displayDeviceInfo(OPT_DeviceList deviceInfoList)
{
	OPT_DeviceInfo*	pDevInfo = NULL;
...
int main()
{
	...
	do
	{
		...
		// 停止拉流 
		// Stop grabbing 
		ret = OPT_StopGrabbing(devHandle);
		if (OPT_OK != ret)
		{
